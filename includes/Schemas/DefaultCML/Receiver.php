...
final class Receiver
{
	...
	{
		add_action(WC1C_PREFIX . 'receiver_' . $this->core()->getId(), [$this, 'receiverHandler'], 10);
	}
	 ...
	 */
	private function responseByType($type = 'failure', $description = '')
	{
		$this->core()->log()->info('Response by type send.', ['type' => $type, 'description' => $description]);

		if($this->core()->getOptions('convert_cp1251', '') === 'yes')
		{
			$description = mb_convert_encoding($description, 'cp1251', 'utf-8');

			$this->core()->log()->info('Response charset converted to Windows-1251', ['description' => $description]);
	 ...
	 * @return bool
	 */
	private function handlerCheckauthKey()
	{
		...
		{
			$this->core()->log()->warning('$_COOKIE[$session_name] is not set.');
			return false;
		}

		if($_COOKIE[$session_name] !== $session_id)
		{
			$this->core()->log()->warning('$_COOKIE[$session_name] error ' . $_COOKIE[$session_name] . ' current ' . $session_id);
			return false;
		}
	 ...
	 * Receiver handler
	 */
	public function receiverHandler()
	{
		if(session_status() === PHP_SESSION_NONE)
		{
			$this->core()->log()->info('Session none, start new session.');
			session_start();
		}
		...
		$this->core()->log()->info('Receiver handler: $type=' . $type . ' $mode=' . $mode);
		...
		if($type === 'catalog' && $mode !== '')
		{
			...
			{
				case 'checkauth':
					$this->handlerCheckauth();
					break;
				case 'init':
					$this->handlerModeInit();
					break;
				case 'file':
					$this->handlerCatalogModeFile();
					break;
				case 'import':
					$this->handlerCatalogModeImport();
					break;
				case 'deactivate':
				case 'complete':
					$this->responseByType('success');
					break;
				default:
					$this->responseByType('success');
			}
		...
		$this->responseByType('failure', __('Schema: action not found', 'wc1c'));
	}

	/**
	 * Checkauth
	 */
	private function handlerCheckauth()
	{
		$user_login = '';
		$user_password = '';
		...
		if(!isset($_SERVER['PHP_AUTH_USER']))
		{
			...
			{
				$this->core()->log('schemas')->critical('Проверьте наличие записи в корневом файле .htaccess после RewriteEngine On:\nRewriteCond %{HTTP:Authorization} ^(.*)\nRewriteRule ^(.*) - [E=HTTP_AUTHORIZATION:%1]');
				$this->core()->configuration()->setStatus('error');
				$this->core()->configuration()->save();
				$this->responseByType('failure', __('Not specified the user. Check the server settings.', 'wc1c'));
			}
		}
		else
		{
		...
		if($user_login !== $this->core()->getOptions('user_login', ''))
		{
			$this->core()->log()->notice(__('Not a valid username.', 'wc1c'));
			$this->responseByType('failure', __('Not a valid username', 'wc1c'));
		}

		if($user_password !== $this->core()->getOptions('user_password', ''))
		{
			$this->core()->log()->notice(__('Not a valid user password.', 'wc1c'));
			$this->responseByType('failure', __('Not a valid user password', 'wc1c'));
		}

		$session_name = session_name();
		$session_id = session_id();

		$this->core()->log()->info(__('Checkauth is success.', 'wc1c'), ['session_name' => $session_name, 'session_id' => $session_id]);
	 ...
	 */
	private function handlerModeInit()
	{
		if($this->handlerCheckauthKey() === false)
		{
			$this->responseByType('failure', __('Authorization failed.', 'wc1c'));
		}

		$zip_support = false;
		if(class_exists('ZipArchive'))
		{
			$this->core()->log()->info('ZipArchive available.');
			$zip_support = true;
		}

		$data[0] = 'zip=no';
		if($zip_support && $this->core()->getOptions('file_zip', 'no') === 'yes')
		{
			$this->core()->log()->info('ZipArchive is set to use.');
			$data[0] = 'zip=yes';
		}

		$manual_size = $this->utilityConvertFileSize($this->core()->getOptions('post_file_max_size', ''));
		$post_max_size = $this->getPostFileSizeMax();
		...
		$this->core()->log()->debug('Mode init is success.', ['mode_init_data' => $data]);
	...
	/**
	 * Выгрузка CommerceML файлов в локальный каталог
	 *
	 * @return void
	 */
	private function handlerCatalogModeFile()
	{
		if($this->handlerCheckauthKey() === false)
		{
			$this->responseByType('failure', __('Authorization failed', 'wc1c'));
		...
		$upload_dir = $this->core()->configuration()->getUploadDirectory() . DIRECTORY_SEPARATOR . 'catalog' . DIRECTORY_SEPARATOR;

		if(!is_dir($upload_dir))
		{
			if(!mkdir($upload_dir, 0777, true) && !is_dir($upload_dir))
			{
				$error = __('Unable to create a directory:', 'wc1c') . ' ' . $upload_dir;
				$this->core()->log()->error($error, ['dir' => $upload_dir]);
				$this->responseByType('failure', $error);
			}
		}
		...
		if($filename === '')
		{
			$this->core()->log()->notice('Filename is empty.');
			$this->responseByType('failure', __('Filename is empty.', 'wc1c'));
		}

		$configuration_upload_file_path = $upload_dir . $filename;

		$this->core()->log()->info('Upload file started.', ['configuration_upload_file_path', $configuration_upload_file_path]);

		if(strpos($filename, 'import_files') !== false)
		{
			$this->core()->log()->info('Clean upload directories.');
			$this->clean_upload_file_tree(dirname($filename), $upload_dir);
		}

		if(!is_writable($upload_dir))
		{
			$this->core()->log()->error('Directory is not writable!', ['directory' => $upload_dir]);
			$this->responseByType('failure', 'Невозможно записать файлы в директорию:' . ' ' . $upload_dir);
		}
		...
		if($file_data !== false)
		{
			$file_size = file_put_contents($configuration_upload_file_path, $file_data, LOCK_EX);

			if($file_size)
			{
				$this->core()->log()->info('File size - ' . $file_size, ['file_size' => $file_size]);

				chmod($configuration_upload_file_path , 0777);

				$this->core()->log()->info('Upload file - ' . $configuration_upload_file_path . ' is success.');

				if(strpos($filename, '.zip') !== false)
				{
					$xml_files_result = $this->extractZip($configuration_upload_file_path);

					if($this->core()->getOptions('delete_zip_files_after_extract', 'no') === 'yes')
					{
						$this->core()->log()->info('Zip archive is deleted.', ['file' => $configuration_upload_file_path]);
						unlink($configuration_upload_file_path);
					}

					if($xml_files_result === false)
					{
						$this->core()->log()->error('Error extract file from zip archive.', ['file' => $configuration_upload_file_path]);
						$this->responseByType('failure');
					}

					$this->core()->log()->info('Архив успешно принят и распакован.', ['file' => $configuration_upload_file_path]);
					$this->responseByType('success', 'Архив успешно принят и распакован.');
				}

				$this->core()->log()->info('Файл успешно принят', ['filename' => $filename]);
				$this->responseByType('success', 'Файл успешно принят - ' . $filename);
			}

			$this->core()->log()->error('Ошибка записи файла.', ['file' => $configuration_upload_file_path]);
			$this->responseByType('failure', 'Не удалось записать файл по пути: ' . $configuration_upload_file_path);
		}

		$this->core()->log()->warning('File is empty.', ['file' => $configuration_upload_file_path]);
		$this->responseByType('failure', 'Пришли пустые данные. Повторите попытку.');
	}

	/**
	 * Catalog import
	 */
	private function handlerCatalogModeImport()
	{
		if($this->handlerCheckauthKey() === false)
		{
			$this->responseByType('failure', __('Authorization failed.', 'wc1c'));
		...
		$this->core()->log()->info('Import catalog started.');

		$filename = wc1c()->getVar($_GET['filename']);

		if($filename === '')
		{
			$this->core()->log()->warning('Filename is empty.');
			$this->responseByType('failure', __('Import filename is empty.', 'wc1c'));
		}

		$file = $this->core()->configuration()->getUploadDirectory() . DIRECTORY_SEPARATOR . 'catalog' . DIRECTORY_SEPARATOR . $filename;

		$this->core()->log()->info('File processing starting.', ['file' => $file, 'filename' => $filename]);
		...
		{
			$this->core()->log()->error('Ошибка обработки файла импорта каталога.', ['exception' => $e, 'filename' => $filename]);
			$this->responseByType('failure', 'Импорт завершен с ошибкой.' . $e->getMessage());
		}

		if($result_file_processing)
		{
			if($this->core()->getOptions('delete_files_after_processing', 'no') === 'yes')
			{
				$this->core()->log()->info('Delete file - ' . $file, ['file' => $file, 'filename' => $filename]);
				unlink($file);
			}

			$this->core()->log()->info('File processing success.', ['file' => $file, 'filename' => $filename]);
			$this->responseByType('success', 'Импорт успешно завершен.');
		}

		$this->core()->log()->error('File processing error.', ['file' => $file, 'filename' => $filename]);
		$this->responseByType('failure', 'Импорт завершен с ошибкой.');
	}
	...
	private function file_processing($file_path)
	{
		...
		{
			throw new Exception('$file_path is not file');
		}

		$type_file = $this->fileTypeDetect($file_path);

		if($type_file === '')
		{
			throw new Exception('$type_file is not valid');
		}

		$this->core()->log()->info('type_file - ' . $type_file);
		...
		{
			throw new Exception('Function libxml_use_internal_errors is not exists.');
		}
		...
		if(!$xml_data)
		{
			$this->core()->log()->error('Xml errors.', ['xml_errors' => libxml_get_errors()]);
			throw new Exception('Xml errors');
		}

		if($this->core()->getOptions('skip_file_processing', 'yes') === 'yes')
		{
			$this->core()->log()->info('File processing skip, end & true.');
			return true;
		}
		...
		{
			$this->core()->log()->info('classifier_processing start');
			...
			{
				$this->core()->log()->error('file_processing: exception - ' . $e->getMessage());
				throw new Exception('file_processing: exception - ' . $e->getMessage());
			}

			$this->core()->log()->info('file_processing: classifier_processing end');
		}
		...
		if($xml_data->Каталог)
		{
			$this->core()->log()->info('file_processing: catalog_processing start');
			...
			{
				$this->core()->log()->info('file_processing: exception - ' . $e->getMessage());
				throw new Exception('file_processing: exception - ' . $e->getMessage());
			}

			$this->core()->log()->info('file_processing: catalog_processing end, success');
		}
		...
		{
			$this->core()->log()->info('file_processing: offers_package_processing start');
			...
			$this->core()->log()->info('file_processing: offers_package_processing end, success');
		}
	...
	private function parse_xml_catalog($xml_catalog_data)
	{
		...
		$this->core()->log()->debug('parse_xml_catalog: $data', $catalog_data);
	...
	private function parse_xml_catalog_products($xml_catalog_products_data)
	{
		...
		foreach($xml_catalog_products_data->Товар as $xml_product_data_key => $xml_product_data)
		{
			...
			{
				$this->core()->log()->info('parse_xml_catalog_products: exception - ' . $e->getMessage());
				continue;
			}
		}
	...
	private function parse_xml_offers_package($xml_offers_package_data)
	{
		...
		{
			$this->core()->log()->info('parse_xml_offers_package: $xml_data->Предложения start');
			...
			{
				$this->core()->log()->info('parse_xml_offers_package: $xml_data->Предложения exception - ' . $e->getMessage());
			}

			$this->core()->log()->info('parse_xml_offers_package: $xml_data->Предложения end');
		}
	 ...
	 * @return string
	 */
	private function fileTypeDetect($file_name)
	{
		$types = ['import_files', 'import', 'offers', 'prices', 'rests'];
	...
	private function parse_xml_classifier_groups($xml_data, $parent_id = '', &$groups = [])
	{
		foreach($xml_data->Группа as $xml_category)
		{
			...
			{
				$this->core()->log()->warning('parse_xml_classifier_groups: ' . $e->getMessage());
				continue;
			}
	...
	private function load_shop_categories()
	{
		...
		$this->core()->log()->debug('load_categories_wc: get_terms $current_categories_query', $current_categories_query);
	...
	private function processing_classifier_groups($classifier_groups = [])
	{
		...
		foreach($classifier_groups as $classifier_group => $classifier_group_value)
		{
			...
			{
				$this->core()->log()->info('processing_classifier_groups: exception - ' . $e->getMessage());
				continue;
			}
	...
	private function parse_xml_classifier_properties($xml_data)
	{
		...
		foreach($properties_xml_data as $property_xml_data)
		{
			...
			{
				$this->core()->log()->info('parse_xml_classifier_properties: exception - ' . $e->getMessage());
			}
		}
	...
	private function parse_xml_classifier($xml_classifier_data)
	{
		...
		$this->core()->log()->info('parse_xml_classifier: classifier_guid ' . $classifier_data['classifier_guid'] . ', classifier_name ' . $classifier_data['classifier_name']);
		...
		{
			$this->core()->log()->info('parse_xml_classifier: classifier_processing_groups start');
			...
			$this->core()->log()->info('parse_xml_classifier: classifier_processing_groups end, success');
		}
		...
		{
			$this->core()->log()->info('parse_xml_classifier: classifier_processing_properties start');
			...
			$this->core()->log()->info('parse_xml_classifier: classifier_processing_properties end, success');
		}
	...
	private function set_attributes_by_classifier_properties($classifier_properties = [])
	{
		...
		foreach($classifier_properties as $classifier_property_id => $classifier_property)
		{
			...
			if($attribute_id === 0)
			{
				...
				{
					$this->core()->log()->warning('set_attributes_by_classifier_properties: exception - ' . $e->getMessage());
					continue;
				}
			...
			if($attribute_id !== 0)
			{
				...
				if($classifier_property['property_values_type'] === 'Справочник')
				{
					foreach($classifier_property['property_values_variants'] as $property_values_variant_key => $property_values_variant_value)
					{
						...
						{
							$this->core()->log()->warning('parse_xml_classifier_properties: exception - ' . $e->getMessage());
						}
	...
	private function extractZip($zip_file_path)
	{
		...
		{
			$this->core()->log()->info('Unpack zip archive start.', ['file' => $zip_file_path]);

			while($zip_entry = zip_read($zip_archive))
			{
				$name = zip_entry_name($zip_entry);

				$this->core()->log()->info('Unpack file.', ['file' => $name]);

				$type_detect = $this->fileTypeDetect($name);
				$this->core()->log()->info('Unpack file by type.', ['file' => $name, 'file_type' => $type_detect]);

				if($type_detect === 'import_files')
				{
					$result = $this->extractZipImage($zip_archive, $zip_entry, substr($name, $type_detect));
				...
				{
					$result = $this->extractZipXml($zip_archive, $zip_entry, $name);
			...
			$this->core()->log()->info('Unpack success.', ['file' => $zip_file_path]);

			zip_close($zip_archive);
		}
		else
		{
			$this->core()->log()->error('Zip_open error.', ['file' => $zip_file_path]);
			return false;
		}

		if($img_files > 0)
		{
			$this->core()->log()->info('Unpack images count - ' . $img_files, ['count_images' => $img_files]);
		}

		if($error_files > 0)
		{
			$this->core()->log()->error('Unpack error files count - ' . $error_files , ['count_images' => $error_files]);
			return false;
		}
	 ...
	 * @return boolean
	 */
	private function extractZipXml($zip_arc, $zip_entry, $name)
	{
		$uploads_files_dir = $this->core()->configuration()->getUploadDirectory() . DIRECTORY_SEPARATOR . 'catalog' . DIRECTORY_SEPARATOR;
		...
		if(substr($name, -1) === '/')
		{
			...
			{
				$this->core()->log()->info('Каталог существует: ' . $name);
			}
			else
			{
				$this->core()->log()->info('Создан каталог: ' . $name);
				if(!mkdir($concurrentDirectory = $uploads_files_dir . $name, 0775, true) && !is_dir($concurrentDirectory))
				{
					return false;
				}
			}
		...
		elseif(zip_entry_open($zip_arc, $zip_entry, "r"))
		{
			...
			if(file_exists($uploads_files_dir . $name))
			{
				...
				$this->core()->log()->info('Удален старый файл: ' . $uploads_files_dir . $name);
			}
			...
			if($fd = @fopen($uploads_files_dir . $name, "wb"))
			{
				...
				$this->core()->log()->info('Создан файл: ' . $uploads_files_dir . $name);
			...
			{
				$this->core()->log()->info('Ошибка создания и открытия на запись: ' . $uploads_files_dir . $name);
			}
	 ...
	 * @return boolean
	 */
	private function extractZipImage($zipArc, $zip_entry, $name)
	{
		$import_files_dir = $this->core()->configuration()->getUploadDirectory() . '/catalog/import_files/';

		if(substr($name, -1) === "/")
		{
			if(!is_dir($import_files_dir . $name))
			{
				if(!mkdir($concurrentDirectory = $import_files_dir . $name, 0777, true) && !is_dir($concurrentDirectory))
				{
					return false;
				}
			}
		...
		elseif(zip_entry_open($zipArc, $zip_entry, "r"))
		{
			 ...
			 * Logger
			 */
			$this->core()->log()->info('Extract image: ' . $name);
			...
			if(is_file($import_files_dir . $name))
			{
				...
				{
					$this->core()->log()->info('Файл: ' . $name . ' существует, но старый! Старый размер ' . $size_file . ', новый ' . $size_dump);
					...
					if($fd === false)
					{
						$this->core()->log()->error('Ошибка открытия файла: ' . $import_files_dir . $name);
						return false;
					}
					...
					$this->core()->log()->info('Файл: ' . $name . ' перезаписан.');
				}
			}
			else
			{
				...
				if($pos !== false)
				{
					$this->core()->log()->error('Ошибка записи файла: ' . $import_files_dir . $name . '! Он является PHP скриптом и не будет записан!');
				}
				else
				{
					$fd = @fopen($import_files_dir . $name, 'wb');
					...
					if($fd === false)
					{
						$this->core()->log()->error('Ошибка открытия файла: ' . $import_files_dir . $name . ' Проверьте права доступа!');
						return false;
					}
					...
					$this->core()->log()->info('Создан файл: ' . $import_files_dir . $name);
				}
			}
		...
		$this->core()->log()->info('Распаковка изображения завершена!');
	...
	private function clean_upload_file_tree($path, $current_dir = false)
	{
		foreach(explode('/', $path) as $name)
		{
			...
			if(!mkdir($concurrentDirectory = $current_dir . $name) && !is_dir($concurrentDirectory))
			{
				continue;
			}
	 ...
	 * @return float|int
	 */
	private function getPostFileSizeMax()
	{
		$size = $this->utilityConvertFileSize(ini_get('post_max_size'));

		$size_max_manual = $this->utilityConvertFileSize($this->core()->getOptions('post_file_max_size'));
